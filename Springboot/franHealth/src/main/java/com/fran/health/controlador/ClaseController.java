...
import com.fran.health.repositorio.ClaseRepositorio;
...
//		
//		//Prueba
		@PostMapping(path="/eliminarClase",consumes=MediaType.APPLICATION_JSON_VALUE)
		public DTO  eliminarmodulo(@RequestBody datosActualizarClase d, HttpServletRequest request) {
				...
				try {
					...
					//Creamos un nuevo usuario y añadimos los valores del JSon en el nuevo objeto usuario
					Clase doc = claseRepo.findById(d.id);
					doc.setClasecol(d.clasecol);
					//Guardamos el usuario pasando el mismo al método save de usuarioRepo
					claseRepo.save(doc);
					//Guardamos en dto, el en indice "0" el valor "Perfecto"
					dto.put("0", "Perfecto");
...
;				}
		...
		@PostMapping(path="/crearClase",consumes=MediaType.APPLICATION_JSON_VALUE)
		public DTO  anadeModulo(@RequestBody datosCrearClase d, HttpServletRequest request) {
				...
				try {
					...
					Clase doc = new Clase();
					doc.setNombre(d.nombre);
					doc.setDescripcion(d.descripcion);
					doc.setImagen(d.foto);
					doc.setHorario(d.horario);
					System.out.println(d.horario);
					//Guardamos el usuario pasando el mismo al método save de usuarioRepo
					claseRepo.save(doc);
					System.out.println(doc.getId());
//					Usuarioclase cls = new Usuarioclase();
//					cls.setIdClase(doc);
//					System.out.println(cls);
//					usuarioClaseRepo.save(cls);
		...
		@PostMapping(path="/crearUsuarioClase",consumes=MediaType.APPLICATION_JSON_VALUE)
		public DTO  anadirModulo(@RequestBody datosActualizarUsuarioClase d, HttpServletRequest request) {
				...
				try {
					...
					//Creamos un nuevo usuario y añadimos los valores del JSon en el nuevo objeto usuario
					Usuarioclase doc = new Usuarioclase();
					doc.setIdClase(claseRepo.findById(d.idclase));
					doc.setIdUsuario(usuarioRepo.findById(d.idmonitor));
					usuarioClaseRepo.save(doc);
		...
		@PostMapping(path="/modificarClase",consumes=MediaType.APPLICATION_JSON_VALUE)
		public DTO  modificarModulo(@RequestBody datosModificarClase d, HttpServletRequest request) {
				...
				try {
					...
					//Creamos un nuevo usuario y añadimos los valores del JSon en el nuevo objeto usuario
					Clase doc = claseRepo.findById(d.idclase);
					doc.setNombre(d.nombre);
					doc.setDescripcion(d.descripcion);
					doc.setImagen(d.foto);
					doc.setHorario(d.horario);
		...
		@PostMapping(path="/modificarUsuarioClase",consumes=MediaType.APPLICATION_JSON_VALUE)
		public DTO  modificarusuclaseModulo(@RequestBody datos2ActualizarUsuarioClase d, HttpServletRequest request) {
				...
				try {
					...
					//Creamos un nuevo usuario y añadimos los valores del JSon en el nuevo objeto usuario
					Usuarioclase doc = usuarioClaseRepo.findById(d.id);
					doc.setIdClase(claseRepo.findById(d.idclase));
					doc.setIdUsuario(usuarioRepo.findById(d.idmonitor));
					usuarioClaseRepo.save(doc);
					System.out.println("Dentro del modificar usuario clase");
		...
		@PostMapping(path="/annadirUsuarioClase",consumes=MediaType.APPLICATION_JSON_VALUE)
		public DTO  anadirUsuClaseModulo(@RequestBody datosAnnadirUsuarioClase d, HttpServletRequest request) {
				...
				try {
					...
					Usuarioclase doc = new Usuarioclase();
					doc.setIdClase(claseRepo.findById(d.idclase));
					doc.setIdUsuario(usuarioRepo.findById(d.idusuario));
					usuarioClaseRepo.save(doc);
		...
		@RequestMapping("/eliminarClase/{id}")
		public DTO elimiarClase(@PathVariable(value="id") int id) {
			
			DTO dto = new DTO();
			System.out.println(id);
			try {
				usuarioClaseRepo.deleteById(id);
				dto.put("0", "correcto");
			} catch (Exception e) {
				dto.put("0", "error");
			}
...
//					listaUsuarios.add(datosUsuario(u));
//				}
//				dto.put("usuarios", listaUsuarios);
...
class datosActualizarClase{
	String clasecol;
	int id;
	
	
	public datosActualizarClase(int id, String clasecol) {
		super();
		this.id = id;
...
class datosCrearClase{
	String nombre, descripcion, horario, monitor;
	byte[] foto;
	
	
	public datosCrearClase(String nombre, String descripcion, byte[] foto, String horario, String monitor) {
		super();
		this.nombre = nombre;
...
class datosModificarClase{
	String nombre, descripcion, horario, monitor;
	byte[] foto;
	int idclase;
	
	
	public datosModificarClase(int idclase, String nombre, String descripcion, byte[] foto, String horario, String monitor) {
		super();
...
class datosActualizarUsuarioClase{
	int idmonitor;
	int idclase;
	
	
	public datosActualizarUsuarioClase(int idclase, int idmonitor) {
		super();
		this.idclase = idclase;
...
class datos2ActualizarUsuarioClase{
	int id;
	int idmonitor;
	int idclase;
	
	
	public datos2ActualizarUsuarioClase(int id, int idclase, int idmonitor) {
		super();
		this.id = id;
		this.idclase = idclase;
		this.idmonitor = idmonitor;
...
class datosAnnadirUsuarioClase{
	int idusuario;
	int idclase;
	
	
	public datosAnnadirUsuarioClase(int idclase, int idusuario) {
		super();
		this.idclase = idclase;
