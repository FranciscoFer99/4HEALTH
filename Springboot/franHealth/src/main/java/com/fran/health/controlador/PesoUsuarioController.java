...
import com.fran.health.repositorio.PesoUsuarioRepositorio;
...
public class PesoUsuarioController {
	...
	@Autowired
	 PesoUsuarioRepositorio pesoUsuarioRepo;
...
//		//Prueba
		...
		@GetMapping("/buscarPesoUsuario/{id}")
		public DTO buscarPesoUsuario(@PathVariable(value="id") int id) {	
			DTO dto = new DTO();
			//Creamos una lista de usuarios para traernos todos los usuarios
			List<Pesousuario>usuarios = pesoUsuarioRepo.findByPesoUsuario(usuarioRepo.findById(id));
//			usuarioRepo.findById(id))
			List<DTO> listaUsuarios = new ArrayList<DTO>();
			
			if (usuarios != null) {
				for(Pesousuario u : usuarios) {
		...
		private DTO datosClaseNoInscritoUsuario(Pesousuario u) {
			DTO dto = new DTO();
			dto.put("idPeso", u.getId());
			dto.put("fecha", u.getFecha());
			dto.put("datosUsuario", u.getIdusuario());
			dto.put("valor", u.getValor());
		...
		@RequestMapping("/eliminarPesoUsuario/{id}")
		public DTO eliminarPesoUSuario(@PathVariable(value="id") int id) {
			
			DTO dto = new DTO();
			try {
				pesoUsuarioRepo.deleteById(id);
				dto.put("0", "correcto");
			} catch (Exception e) {
				dto.put("0", "error");
			}
		...
		@PostMapping(path="/crearPesoUsuario",consumes=MediaType.APPLICATION_JSON_VALUE)
		public DTO  anadirPesoModulo(@RequestBody datosCrearPesoUsuario d, HttpServletRequest request) {
				...
				try {
					...
					//Creamos un nuevo usuario y añadimos los valores del JSon en el nuevo objeto usuario
					Pesousuario doc = new Pesousuario();
					doc.setValor(d.valor);
					doc.setFecha(d.fechaActual);
					doc.setIdusuario(usuarioRepo.findById(d.idusuario));
					pesoUsuarioRepo.save(doc);
...
class datosCrearPesoUsuario{
	int idusuario;
	String  valor;
	Date fechaActual;
	
	
	public datosCrearPesoUsuario(String valor, int idusuario, Date fechaActual) {
		super();
		this.valor = valor;
